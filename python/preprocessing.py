...
def statistics(filepath, nBDs, rel_unc, relM, f, gamma, rs, rank=100, D=2):
    ...
    for i in range(rank):
        ...
        file_name = (filepath + ("N%irelT%.2frelM%.2f/posterior_ex3v_N%i_relunc%.2f_relM%.2f_f%.1fgamma%.1frs%.1fv%i" 
    ...
    output = open(filepath + ("statistics_ex3v_N%i_relunc%.2f_relM%.2f_f%.1fgamma%.1frs%.1f" 
...
if __name__ == '__main__':
    ...
    filepath = "/scratch/mariacst/exoplanets/results/robustness/DMvelocity/"
    nBDs     = [1000]
    rel_unc  = [0.1]
    rel_M    = [0.1]
    f        = 1.

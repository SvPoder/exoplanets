...
def mock_population_all_check(N, relT, relM, relRobs, relA,                           
                        f_true, gamma_true, rs_true, rho0_true=0.42,            
                        Tmin=0., v=None):                                       
    """                                                                         
    Generate N observed exoplanets                                              
    ...
    if len(pos[0]) < N:                                                         
        sys.exit("Less objects than required!")                                 
    #return                                                                     
    return (robs_wn[pos][:N], r_obs[pos][:N], relRobs*r_obs[pos][:N],                           
            Tobs_wn[pos][:N], Tobs[pos][:N], relT*Tobs[pos][:N],                               
            mass_wn[pos][:N], mass[pos][:N], relM*mass[pos][:N],                               
            ages_wn[pos][:N], ages[pos][:N], relA*ages[pos][:N])  

def mock_population_all_plot(N, relT, relM, relRobs, relA,                           
                        f_true, gamma_true, rs_true, rho0_true=0.42,            
                        Tmin=0., v=None):                                       
    """                                                                         
    Generate N observed exoplanets                                              
    ...
    # Mass                                                                      
    mass = IMF_sampling(-0.6, _N, Mmin=35, Mmax=55) # [Mjup]                     
    mass = mass*M_jup.value/M_sun.value # [Msun]                                
    # load theoretical BD cooling model - ATMO 2020                             
    path =  "/home/mariacst/exoplanets/running/data/"                           
    data = np.genfromtxt(path + "./ATMO_CEQ_vega_MIRI.txt", unpack=True)        
...
def mock_population_all_Asimov_1sigma(N, relT, relM, relR, relA,                       
                          f_true, gamma_true, rs_true, rho0_true=0.42,          
                          Tmin=0., v=None):                                     
    """                                                                         
    Generate N observed exoplanets                                              
    ...
    if len(pos[0]) < N:                                                         
        sys.exit("Less objects than required!")                                 
    #return                                                                     
    return (robs[pos][:N]+relR*robs[pos][:N], relR*robs[pos][:N],                                  
            Tobs[pos][:N]+relT*Tobs[pos][:N], relT*Tobs[pos][:N],                                  
            mass[pos][:N]+relM*mass[pos][:N], relM*mass[pos][:N],                                  
            ages[pos][:N]+relA*ages[pos][:N], relA*ages[pos][:N])  

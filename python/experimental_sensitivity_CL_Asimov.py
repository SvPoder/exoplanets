...
from mock_generation_new import mock_population_all, mock_population_all_Asimov
import numpy as np
...
def UL_at_rs(rs, f, nBDs, relT, relM, relR, relA, points, values,
    ...
    # Check p values
    print("p_values at gamma_up = {0}, rs = {1}, p_values = {2} {3}".format(gamma_up, rs, _p_sb, _p_b))
...
if __name__=="__main__":
    ...
    seed      = 250
    np.random.seed(seed)
    (robs, sigmarobs, Tobs, sigmaTobs, Mobs, sigmaMobs, Aobs, sigmaAobs) = mock_population_all_Asimov(nBDs, relT,
    ...
    with Pool(4) as p:
        results = p.map(partial(UL_at_rs,f = f,
                                         ...
                                         points = points,
                                         values = values,
                                         robs = robs, sigmarobs = sigmarobs, Mobs = Mobs, sigmaMobs = sigmaMobs, Aobs = Aobs,
                                         sigmaAobs = sigmaAobs, Tobs = Tobs, sigmaTobs = sigmaTobs, Teff = Teff,
                                         dervTint_M = dervTint_M, dervTint_A = dervTint_A, steps = steps, v=v, gamma_min = gamma_min[0], gamma_max = gamma_max[0]), rs)


    '''
    for i, _rs in enumerate(rs):
    ...
    '''
    for i, gamma in enumerate(list(results)):
        gamma_up[i] = gamma

    #print(gamma_up)
    output = np.array((np.array(rs), gamma_up))
    # save results
    np.savetxt("MULTI_UL_" + ex + "_nBDs%i_f%.1f_steps%i_sigma%.1f_rs%.1fAsimov.dat"
               %(nBDs, f, steps, sigma, 20),
               output.T, fmt="%.4f  %.4f")
